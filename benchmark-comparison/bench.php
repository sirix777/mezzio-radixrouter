...
use Sirix\Router\RadixRouter\BenchmarkComparison\MezzioFastRouteAdapter;
use Sirix\Router\RadixRouter\BenchmarkComparison\MezzioFastRouteCachedAdapter;
use Sirix\Router\RadixRouter\BenchmarkComparison\MezzioLaminasRouterAdapter;
use Sirix\Router\RadixRouter\BenchmarkComparison\MezzioRadixRouterAdapter;
use Sirix\Router\RadixRouter\BenchmarkComparison\MezzioRadixRouterCachedAdapter;
...
function runBenchmark(
    ...
    $url =
        "http://127.0.0.1:$port/?" .
        http_build_query(array_filter($params, fn($v) => $v !== null));
    $json = file_get_contents($url);
    $data = json_decode($json, true);

    if ($json === false || !is_array($data)) {
        throw new RuntimeException(
            "Failed to get benchmark result from $url : $json",
        );
    }
    if (isset($data["error"])) {
...
function warmupServers(
    ...
    $totalWarmups =
        count($allTestSuites) * count($allRouters) * count($benchmarkModes);
    $warmupCount = 0;
...
function runBenchmarks(
    ...
    foreach ($allTestSuites as $suite) {
        foreach ($allRouters as $routerClass) {
            $routerName = method_exists($routerClass, "details")
                ? $routerClass::details()["name"]
                : $routerClass;
            foreach ($benchmarkModes as [$modeLabel, $phpOpts]) {
                $port = $servers[$modeLabel]["port"];
                $result = runBenchmark(
...
function benchmarkRegistrationTimes(
    ...
    foreach ($results as &$row) {
        ...
        $avgRegister = $registerTimes
            ? array_sum($registerTimes) / count($registerTimes)
            : null;
...
function printUsageScreen($cliSuites, $cliRouters, $cliModes)
{
    echo "\nUsage: " .
        __FILE__ .
        " [--suite=\"simple, avatax, bitbucket\"] [--router=\"YiiRadixRouterAdapter, FastRouteAdapter,...\"] [--mode=\"JIT=tracing, OPcache, No OPcache\"] [--all]\n";
    echo "\nOptions:\n";
...
$cliRouters = [
    "MezzioFastRouteAdapter" => MezzioFastRouteAdapter::class,
    "MezzioFastRouteCachedAdapter" => MezzioFastRouteCachedAdapter::class,
    "MezzioLaminasRouterAdapter" => MezzioLaminasRouterAdapter::class,
    "MezzioRadixRouterAdapter" => MezzioRadixRouterAdapter::class,
    "MezzioRadixRouterCachedAdapter" => MezzioRadixRouterCachedAdapter::class,
];
$cliModes = [
    "JIT=tracing" => [
        "JIT=tracing",
        "-d zend_extension=opcache -d opcache.enable=1 -d opcache.enable_cli=1 -d opcache.jit_buffer_size=100M -d opcache.jit=tracing",
    ],
    "OPcache" => [
        "OPcache",
        "-d zend_extension=opcache -d opcache.enable=1 -d opcache.enable_cli=1",
    ],
...
if (isset($opts["all"])) {
    ...
    $hasAnyOption =
        isset($opts["suite"]) || isset($opts["router"]) || isset($opts["mode"]);
    if (!$hasAnyOption) {
...
echo "Selected routers: " .
    implode(
        ", ",
        array_map(function ($r) use ($cliRouters) {
